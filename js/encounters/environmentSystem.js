...
export const ENVIRONMENTS = {
    ...
    arcaneNexus: {
        ...
        onDiceRoll(dice, isPlayer, combat) {
            ...
            const modified = [...dice];
            // Max value = the die size (for enemy: combat.enemy.dice[0]; for player, use die object's .max)
            const maxValue = isPlayer
                ? (combat.player.dice[0]?.max || dice[0])
                : ([...combat.enemy.dice, ...combat.enemy.extraDice][0] || dice[0]);
            modified[0] = maxValue;

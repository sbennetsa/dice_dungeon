...
- **Player advantage values**: All advantage values (rests, shops, events, artifacts) use **threat-equivalent units** — same scale as enemy `baseThreat`. Anchored on die upgrade (+1/+1 ≈ 5% DPS boost × remaining combat threat). Heal (30% HP) has the same face value as upgrade — equivalent maintenance choices. Rest total ≈ 3.75× upgrade. See `docs/decisions.md` for full methodology.
- **Shop advantage scales by act**: `SHOP_ADVANTAGES = [20, 40, 55]` (Act 1/2/3). Early shops are worth less because players have had fewer combats and less gold to spend.
